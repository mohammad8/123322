...
local function action_by_id(extra, success, result)  -- /info <ID> function
 ...
 if result.username then
   ...
   local text = 'Full name : '..(result.first_name or '')..' '..(result.last_name or '')..'\n'
               ..'Username: '..Username..'\n'
               ..'ID : '..result.id..'\n\n'
  local hash = 'loc:'..extra.chat2..':variables'
  local value = redis:hget(hash, result.id)
  if not value then
  if result.id == tonumber(Arash) then
    text = text..'loc : karaj \n\n'
   elseif is_admin2(result.id) then
    text = text..'loc : notset\n\n'
   elseif is_owner2(result.id, extra.chat2) then
    text = text..'loc : not set\n\n'
   elseif is_momod2(result.id, extra.chat2) then
    text = text..'loc: not set \n\n'
   else
    text = text..'loc : no set\n\n'
   end
   else
    text = text..'loc : '..value..'\n\n'
  end
...
local function run(msg, matches)
 if matches[1]:lower() == 'setoc' or matches[1]:lower() == 'تنظیم شهر' then
 ...
 if matches[1]:lower() == 'info' or matches[1]:lower() == 'اینفو' and not matches[2] then
  ...
  if msg.from.username then
   ...
   local hash = 'loc:'..msg.to.id..':variables'
 if hash then
